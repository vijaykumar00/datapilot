...
class FileRecord:
    def __init__(self, file_id: str, filename: str, df: pd.DataFrame, path: Path):
...
class FileManager:
    ...
    def _read_file(self, path: Path, ext: str, file_size: int) -> pd.DataFrame:
        """Read CSV or Excel into a DataFrame, using chunks for large CSVs."""
        if ext == ".csv":
            if file_size > CHUNK_THRESHOLD:
                logger.info(f"Large file ({file_size // 1024}KB) — using chunked reading")
    ...
    async def process_upload(self, upload: UploadFile) -> dict[str, Any]:
        ...
        # Parse
        try:
            df = self._read_file(save_path, ext, len(raw))
        except Exception as e:
            save_path.unlink(missing_ok=True)
            raise ValueError(f"Could not parse file: {e}")

        # Store
        record = FileRecord(file_id, filename, df, save_path)
        self._cache[file_id] = record
        self._store.register_dataframe(record.table_name, df)

        logger.info(f"Processed '{filename}' → file_id={file_id}, {len(df)} rows")
        return self._build_summary(df, file_id, filename)
